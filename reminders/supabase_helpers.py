...
def get_user_supabase_client(request):
    user_jwt = request.COOKIES.get('jwt') or request.session.get('supabase_jwt')
    if not user_jwt or request.session.get('jwt_expiry', 0) < time.time() + 300:  # Refresh 5 min before expiry
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                request.session['jwt_expiry'] = time.time() + data.get("expires_in", 3600)
